...
class MailService extends Egg.Service {
  async sendMail(data) {
  ...
  async sendActiveMail(who, token, name) {
    ...
    const to = who;
    const subject = config.name + 'HFT帐号激活';
    const html = '<p>您好：' + name + '</p>' +
    '<p>我们收到您在' + config.name + 'HFT交易平台的注册信息，请点击下面的链接来激活帐户：</p>' +
    '<a href  = "' + config.host + '/active_account?key=' + token + '&name=' + name + '">激活链接</a>' +
    '<p>若您没有在' + config.name + 'HFT平台填写过注册信息，说明有人滥用了您的电子邮箱，请删除此邮件，我们对给您造成的打扰感到抱歉。</p>' +
    '<p>' + config.name + ' 谨上。</p>';
  ...
  async sendResetPassMail(who, token, name) {
    ...
    const from = `${config.name} <${config.mail_opts.auth.user}>`;
    const to = who;
    const subject = config.name + 'HFT交易平台密码重置';
    const html = '<p>您好：' + name + '</p>' +
    '<p>我们收到您在' + config.name + 'HFT交易平台重置密码的请求，请在24小时内单击下面的链接来重置密码：</p>' +
